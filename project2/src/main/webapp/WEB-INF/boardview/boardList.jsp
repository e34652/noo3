...
<body>
	...
	<div class="container" style="padding-top: 50px;">
		<table class="table table-bordered table-hover">
			...
			<c:forEach var="board" items="${ArticleB.contentB}">
				<tr>
					<td>${board.num}</td>
					<td><a href="boardView.do?num=${board.num}"> <c:out
								value="${board.title}" />
					</a></td>
					<td>${board.name}</td>
					<td>${board.regtime}</td>
			...
			<c:if test="${ArticleB.hasArticles()}">
				<tr>
					<td colspan="5"><c:if test="${ArticleB.startPage > 5}">
							<a href="boardList.do?pageNo=${ArticleB.startPage - 5}">[이전]</a>
						</c:if> <c:forEach var="pNo" begin="${ArticleB.startPage}"
							end="${ArticleB.endPage}">
							<a href="boardList.do?pageNo=${pNo}">[${pNo}]</a>
						</c:forEach> <c:if test="${ArticleB.endPage < ArticleB.totalPages}">
	...
	<div class="container" style="text-align: right">
		<form class="d-flex" role="search" action="boardSearch.do">
			<select class="form-select" name="selectedOption" aria-label="Default select example"
				style="width: 130px;">
				<option selected value="none">검색 옵션</option>
            ...
            <input type="button" value="글쓰기" class="btn btn-primary" onclick="location.href='boardWrite.do'" style="margin-left: auto;">
		</form>
	</div>
	<script>
